...
export function apiHeaderValue(value, name = "API key") {
  ...
  if (normalized.length !== raw.length) {
    console.warn("PROVIDER_KEY_NORMALIZED", { name, originalLength:raw.length, normalizedLength:normalized.length });
  }
  return normalized;
}

async function capabilitySecret(env) {
  return requiredBinding(env.EBC_VIDEO_CAPABILITY_TOKEN, "EBC_VIDEO_CAPABILITY_TOKEN", 500);
}
...
async function relayToken(env, room) {
  ...
  return `ebc1.${expiresAt}.${signature}`;
}
...
async function authorizeRelay(request, env, room) {
  ...
  if (version !== "ebc1" || !Number.isInteger(expiresAt) || expiresAt < now || expiresAt > now + RELAY_TTL_SECONDS + 60 || !signature) return false;
...
export function normalizeSession(body = {}) {
  ...
  if (tenantId !== TENANT_ID) throw new Error("tenantId must be ebc");
  ...
  return {
    ...
    creatorId,
    fanId,
    creatorName:String(body.creatorName || body.creator_name || "EILA Support").trim().slice(0, 120),
    creatorSlug:String(body.creatorSlug || body.creator_slug || creatorId).trim().slice(0, 120),
    fanName:String(body.fanName || body.fan_name || "EBC operator").trim().slice(0, 120),
    avatarProvider,
    avatarSource,
    agentId,
    imageUrl,
...
async function createSession(request, env) {
  ...
  const supplied = String(request.headers.get("x-ebc-video-capability-token") || "");
  if (!supplied || supplied !== await capabilitySecret(env)) return json({ ok:false, error:"Unauthorized" }, 401);

  const input = normalizeSession(body);
  const livekit = await liveKitConfig(env);
  const room = `ebc-${input.creatorId}-${Date.now().toString(36)}-${crypto.randomUUID().slice(0, 6)}`;
  ...
  console.log("EBC_VIDEO_SESSION_REQUEST", { room, creatorId:input.creatorId, agentName:livekit.agentName });
  const dispatchClient = new AgentDispatchClient(livekit.httpUrl, livekit.apiKey, livekit.apiSecret);
  const dispatch = await dispatchClient.createDispatch(room, livekit.agentName, { metadata:JSON.stringify(metadata) });
  const participant = new AccessToken(livekit.apiKey, livekit.apiSecret, {
    identity:`ebc-operator-${input.fanId}`.slice(0, 120),
    ttl:3600,
    name:input.fanName,
  });
  participant.addGrant({ roomJoin:true, room, canPublish:true, canSubscribe:true, canPublishData:true });
  console.log("EBC_VIDEO_DISPATCH_OK", { room, dispatchId:dispatch?.id || null, agentName:livekit.agentName });
  return json({
    ok:true,
    mode:"browser",
...
async function relayLemonSlice(request, env, url) {
  ...
  if (tenantId !== TENANT_ID || !room) return json({ ok:false, error:"tenant=ebc and room are required" }, 400);
  if (!(await authorizeRelay(request, env, room))) return json({ ok:false, error:"Unauthorized" }, 401);
  const providerKey = apiHeaderValue(
    await requiredBinding(env.LEMONSLICE_EBC_API_KEY, "LEMONSLICE_EBC_API_KEY", 1000),
    "LEMONSLICE_EBC_API_KEY",
  );
  ...
  console.log("EBC_LEMONSLICE_STATUS", { room, status:upstream.status });
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && (url.pathname === "/" || url.pathname === "/health")) {
      const [livekitApiKey, livekitApiSecret, lemonSliceKey] = await Promise.all([
        ...
        bindingValue(env.LEMONSLICE_EBC_API_KEY, 1000).catch(() => ""),
      ]);
      return json({
        ok:true,
        service:"ebc-video-worker",
        version:"ebc-owned-v1",
        tenant:TENANT_ID,
    ...
    try {
      if (request.method === "POST" && url.pathname === "/internal/video/session") return await createSession(request, env);
      if (request.method === "POST" && url.pathname === "/internal/lemonslice/sessions") return await relayLemonSlice(request, env, url);
      return json({ ok:false, error:"route not found" }, 404);
    } catch (error) {
      console.error("EBC_VIDEO_WORKER_ERROR", error instanceof Error ? error.message : String(error));
      return json({ ok:false, error:error instanceof Error ? error.message : "EBC video worker failure" }, 500);
    }
  },
};
