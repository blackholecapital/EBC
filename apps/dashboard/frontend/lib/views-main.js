...
function contactsRows(list) {
  return (list||state.contacts).map(c=>`<tr>
    ...
    <td>${c.estimateNumber?`<span class="badge badge-success">${esc(c.estimateNumber)}</span><div class="table-meta" style="margin-top:4px">${esc(c.estimatedMonthlyTotal?contactMoney(c.estimatedMonthlyTotal):"Sent")} project</div>`:c.salesHandoffStatus?`<span class="badge badge-danger">Project team ${esc(c.salesHandoffStatus)}</span>`:'<span class="badge badge-muted">Pending</span>'}</td>
...
async function openAiWorkflow(id) {
  ...
  body.innerHTML=`
    ...
    <div class="card" style="margin-bottom:16px"><div class="card-header"><h3>Latest call transcript</h3></div>${latest?.transcript?.length?latest.transcript.map(turn=>`<div style="padding:9px 0;border-bottom:1px solid var(--c-border)"><strong>${turn.role==="buddy"?"EBC AI":"Customer"}:</strong> ${esc(turn.text)}</div>`).join(""):'<div class="table-meta">No transcript is available.</div>'}</div>
