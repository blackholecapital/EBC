...
function eventLabel(event) {
  ...
  if (type === "buddy.turn.completed") return "EBC AI responded";
...
function eventMessage(event) {
  ...
  if (type === "email.sent") return p.subject || "EBC AI email sent";
  if (type === "sms.sent" || type === "sms.delivered") return p.messageType ? `${p.messageType} message` : "EBC AI SMS";
...
renderConversations = async function renderBuddyConversations() {
  ...
  if (!calls.length) {
    return `<div class="page-header"><h2>Conversations <span class="badge badge-muted">0 voice calls</span></h2></div>
      <div class="card"><div class="empty-state"><div class="empty-state-title">No EBC AI voice transcripts captured yet</div><div class="empty-state-text">Voice calls will appear here from the communication event stream.</div></div></div>`;
  }
      ...
      <div class="split-list" id="buddy-call-list">
        ${calls.map(c => {
          ...
          return `<div class="split-item${active?" active":""}" data-action="select-buddy-call" data-id="${esc(c.callSid || "")}" role="option" aria-selected="${active}">
            ...
            <div class="split-item-preview">${esc(product || c.callSid || "EBC AI call")}</div>
          </div>`;
        }).join("")}
      </div>
        ...
        <div class="message-list" id="convo-message-list" style="padding:var(--sp-3)">
          ${(selected.transcript || []).length ? selected.transcript.map(turn => `
            <div class="message ${turn.role === "customer" ? "inbound" : "outbound"}" style="margin-bottom:var(--sp-2)">
              ...
              <div class="message-meta"><strong>${turn.role === "customer" ? "Customer" : "EBC AI"}</strong> · ${fmtDate(turn.at)}</div>
    ...
    <div class="card">
      ${rows.length ? `<div class="table-wrap"><table><thead><tr><th>Time</th><th>Source</th><th>Customer</th><th>Type</th><th>Event</th><th>Details</th></tr></thead><tbody>
        ${rows.map(row => `<tr>
          ...
          <td><span class="badge ${row.source === "EBC AI" ? "badge-info" : "badge-muted"}">${esc(row.source)}</span></td>
          ...
          <td style="min-width:220px"><strong>${esc(row.source === "EBC AI" ? eventLabel({type:row.type}) : row.type)}</strong>${row.message && row.message !== row.type ? `<div class="table-meta" style="margin-top:3px;white-space:normal">${esc(row.message)}</div>` : ""}</td>
