...
async function processSalesHandoff(env,payload={}){
  ...
  const estimateReview=String(payload.messageType||"")==="ebc-estimate-review-requested";
  const patch={stage:"Engaged",callStatus:estimateReview?"Estimate review requested":"Needs sales follow-up",estimateStatus:estimateReview?"Review requested":contact.estimateStatus||"",salesHandoffStatus:"Open",salesHandoffAt:createdAt,salesHandoffReason:reason,requirementsSummary:requirements,lastCallSid:String(payload.callSid||"")};
  await persistContact(env,contact,patch);
  await updateDashboardContact(env,contactId,patch);
  let email={ok:false,skipped:true};
  if(payload.notifyCustomer===true&&contact.email){
    const callNowUrl=await buildCallNowUrl(env,contactId);
    email=await callBinding(env.EMAIL,"https://email.internal/internal/send",{contactId,contact:{...contact,...patch},messageType:String(payload.messageType||"ebc-estimate-review-requested"),requirements,handoff:{status:"Open",createdAt,reason},callback:{callNowUrl,persistent:true,source:"estimate-review-email"}});
  }
  await emit(env,{type:"sales.handoff.created",contactId,callSid:String(payload.callSid||""),message:`Sales handoff created: ${reason}`,reason,requirements,emailOk:email?.ok===true,source:String(payload.source||"ebc-voice-worker")});
  return{ok:true,contactId,handoff:{status:"Open",createdAt,reason,requirements},email};
}
...
async function processSalesAppointment(env,payload={}){
  ...
  const appointmentType=String(payload.appointmentType||contact.appointmentType||"phone-consultation");
  ...
  if(action!=="request"){
    ...
    const appointmentName=appointmentType==="bring-in-evaluation"?"in-shop cart evaluation":"phone consultation";
    if(saved.smsConsent!==false&&saved.phone)sms=await callBinding(env.SMS,"https://sms.internal/internal/send",{contactId,contact:saved,messageType:`ebc-sales-appointment-${action}`,message:`Hi${saved.firstName?` ${saved.firstName}`:""}, your ${brandName(env)} ${appointmentName} is ${verb} for ${label}. Reply EBC any time to reconnect with ${assistantName(env)}. Reply STOP to opt out.`});
    if(saved.email)email=await callBinding(env.EMAIL,"https://email.internal/internal/send",{contactId,contact:saved,messageType:`ebc-sales-appointment-${action}`,appointment:{type:appointmentType,status,start:startIso,end:endIso,timeZone,label,notes:patch.appointmentNotes}});
  ...
  await emit(env,{type:eventType,contactId,appointmentRequestId:requestId,appointmentRequestCount:requestCount,appointmentType,appointmentStatus:status,appointmentStart:startIso,appointmentEnd:endIso,timeZone,label,notes:patch.appointmentNotes,smsOk:sms?.ok===true,emailOk:email?.ok===true,source:String(payload.source||"ebc-workflow")});
  return{ok:true,contactId,appointment:{requestId,requestCount,type:appointmentType,status,start:startIso,end:endIso,timeZone,label,notes:patch.appointmentNotes,notificationStatus:saved.appointmentNotificationStatus},sms,email};
}
