...
export function docusignConsentUrl(env) {
  ...
  const redirectUri = String(
    env.DOCUSIGN_CONSENT_REDIRECT_URI || env.PUBLIC_BASE_URL || "https://ebc-concierge-worker.cryptocapitalgroupfl.workers.dev"
...
export async function createBuddySigningSession(env, {
  ...
  const clientUserId = `ebc-${contactId || contact.id || Date.now()}`;
  const agreementId = `EBC-DEMO-${Date.now()}`;
  const publicBase = String(
    env.PUBLIC_BASE_URL || "https://ebc-concierge-worker.cryptocapitalgroupfl.workers.dev"
  ).replace(/\/$/, "");
  ...
  const envelope = buildDemoEnvelope({
    ...
    selectionNumber,
    agreementId,
    brandName: String(env.BRAND_NAME || "Everything Built Custom"),
    assistantName: String(env.ASSISTANT_NAME || "EBC AI"),
  });
  const signer = envelope?.recipients?.signers?.[0];
  if (!signer) throw new Error("EBC demo envelope is missing a signer");
