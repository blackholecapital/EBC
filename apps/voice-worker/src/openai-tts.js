...
export async function openAiTwilioAudio(env, text) {
  ...
  const voice = String(env.BUDDY_OPENAI_TTS_VOICE || "onyx").trim();
  const speed = Number(env.BUDDY_OPENAI_TTS_SPEED || 1);
  const instructions = String(
    env.BUDDY_OPENAI_TTS_INSTRUCTIONS ||
      "Speak as a warm, confident American man representing Everything Built Custom. Sound like an experienced custom-cart project specialist: natural, practical, conversational, and never like an announcer or phone menu. Use clear phrasing, a relaxed pace, and subtle pauses."
  ).trim();
